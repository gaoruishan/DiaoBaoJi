...
/**
 * 打印工具类 用法介绍： 设置日志打印级别LogUtil.LEVEL = LogUtil.W; (默认为LogUtil.I)
 * 
 * 设置应用打印日志路径名称 LogUtil.setDefaultFilePath(context);
 * 设置应用自定义打印日志路径名称ogUtil.setLogFilePath("com/demo/log"); 设置同步打印记录到日志
 * LogUtil.setSyns(true); 设置启用一个新的LOG LogUtil.startNewLog(); 打印单行信息
 * LogUtil.i(msg) LogUtil.v(msg) LogUtil.w(msg) LogUtil.d(msg) LogUtil.e(msg);
 * 打印异常信息 LogUtil.i(exp); 强制打印信息 LogUtil.print(msg); LogUtil.print(exp);
 * 
 * 
 * @author 功能点： 可以控制打印输出 可以简便打印，经过包装得到精准特定格式的日志信息 可以直接打印异常 可以定位异常信息 可以便捷统一修改，优化
 *         同步输出日志到本地sdcard文件 需要注册权限： 读写SD卡权限
 *         android.permission.WRITE_EXTERNAL_STORAGE 网络权限
 *         android.permission.INTERNET 说明： 1.
 *         日志是针对天数级别的，自动生成的日志名称yyyyMMdd.log形式。 如果当天的日志需要新开一个（比如：日志很大了，需要从新生成一个）
 */
public class Logger {
