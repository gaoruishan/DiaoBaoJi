...
/**
 * ContentResolver类，为访问其它应用程序的对外共享数据提供方法，如上面获取、设置亮度时使用的System.getInt()，System.
 * setInt()。 Activity类，android程序的主要类，一个界面必有此类提供后台支持，需要继承此类。
 * Settings类，android程序系统相关的设置类，各种设置都可在此找。
 * LayoutParams类，android界面相关的参数，如高度、宽度、亮度等。 　　Toast类，一个可自动消失的提示框，轻型控件。
 * 
 * 
 */
