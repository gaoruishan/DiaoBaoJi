package com.zjwy.tiaobaojinew.update;
...
public class UpdateManager {
	...
	public void checkAppUpdate(Context context, final boolean isShowMsg) {
		...
		new Thread() {
			public void run() {
				...
				try {
					Update update = HttpUtil.paresXML();// 解析网络是否更新，返回对象
					msg.what = 1;
					msg.obj = update;
				} catch (Exception e) {
					e.printStackTrace();
	...
	private void getCurrentVersion() {
		try {
			...
			System.out.println("----" + curVersionCode);
	...
	private Runnable mdownApkRunnable = new Runnable() {
		...
		public void run() {
			try {
				String apkName = "DiaoBaoJiApp_" + mUpdate.getVersionName()
						+ ".apk";
				String tmpApk = "DiaoBaoJiApp_" + mUpdate.getVersionName()
						+ ".tmp";
				// 判断是否挂载了SD卡
				...
				if (storageState.equals(Environment.MEDIA_MOUNTED)) {
					savePath = Environment.getExternalStorageDirectory()
							.getAbsolutePath() + "/DiaoBaoJi/Update/";
